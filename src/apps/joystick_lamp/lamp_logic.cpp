...
void Police::render(PixelView& dst, ProgramDuration t)
{
    const uint32_t phase = t.ms % (2 * COLOR_PHASE_MS);
    const float hue = phase < COLOR_PHASE_MS ? 240.0f : 0.0f;  // blue, then red
    const bool lit = t.ms % FLASH_PERIOD_MS < FLASH_PERIOD_MS / 2;
