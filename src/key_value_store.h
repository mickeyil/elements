...
// Big enough for a Wi-Fi credential (32B SSID, 63B passphrase) with margin.
...
public:
    ...
    // Store a NUL-terminated value. False if longer than KEY_VALUE_STORE_MAX_VALUE_SIZE.
    virtual bool put_str(const char* key, const char* value) = 0;

    // Copy the string at key into out (cap bytes), NUL-terminated. Returns:
    //   >= 0  length excluding NUL.
    //   <  0  missing key, type mismatch, or buffer too small.
