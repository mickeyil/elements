...
and `src/deprecated/controller_device.h` still carry the v2 names `t_rel` and `t0_us`
(plus `DeviceFrame::t_rel` / `ProgramFrame::t_rel`). These go away with
the deprecated-directory deletion tracked in `TODO.md`.
