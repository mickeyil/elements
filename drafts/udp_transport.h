...
// -- UDP has no connection state to lose. close() is the only call
// that releases the bound port. Callers that want to recover from a
// send error by re-binding must call close() then bind() themselves.
//
...
public:
    ...
    //
    // Rebind semantics:
    //   - bind(p) while already bound to p: no-op, returns true.
    //   - bind(p) while bound to a different port: returns false; the
    //     caller must close() first.
    //   - bind(0) while already bound (to any port): no-op against the
    //     already-assigned ephemeral port, returns true. Treating this
    //     as "give me a new port" would force impls to silently rotate
    //     sockets, which no caller wants.
