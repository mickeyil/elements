...
- Python tests run via the managed venv: `local/venv/bin/python -m pytest`. The venv lives at `local/venv` (created by `./elemctl setup`); there is no `.venv`, and system pythons don't have pytest.
